function triangle(i::Integer, parent::ArbField = RealField(64); withinterior = true)
    ...
    elseif i == 7
        if withinterior
            us = [
                SphericalVertexEigenfunction(domain, 1, stride = 2),
    ...
    elseif i == 8
        if withinterior
            us = [
                [SphericalVertexEigenfunction(domain, i, stride = 2) for i = 1:3]
                SphericalInteriorEigenfunction(domain, stride = 1)
            ] #6
            u = SphericalCombinedEigenfunction(domain, us, [1, 1, 1, 4])
        else
            us = [[SphericalVertexEigenfunction(domain, i, stride = 2) for i = 1:3]]
            u = SphericalCombinedEigenfunction(domain, us, [1, 1, 1])
        end
    elseif i == 9
        if withinterior
            us = [
                SphericalVertexEigenfunction(domain, 2),
